...
public interface HockeyPlayerStatsArchiveRepository extends JpaRepository<HockeyPlayerStatsArchive, Long>{
	...
	@Query(nativeQuery = true, value = "select * from hockeyPlayerStatsArchive where playerId in(select playerId from hockeyPlayer where playerId in (select playerId from memberTeam where memberId in (select id from leagueMember where userid = :userid and leagueId = :leagueid)))")
